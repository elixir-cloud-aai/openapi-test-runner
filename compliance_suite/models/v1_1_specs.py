...
#   timestamp: 2023-01-23T23:04:50+00:00
#   command: datamodel-codegen --input compliance_suite/models/task_execution_service.openapi.yaml --output compliance_suite/models/v1_1_specs.py
...
class TesExecutorLog(BaseModel):
    ...
    stdout: Optional[str] = Field(
        ...
        description=(
            ...
            " path, and use Task.outputs to upload that file\nto permanent storage."
        ),
    )
    stderr: Optional[str] = Field(
        None,
...
class TesOutput(BaseModel):
    ...
    url: str = Field(
        ...
        description=(
            ...
            " contains wildcards, it must be a directory; see\n`tesOutput.path_prefix` for details on how output URLs"
            " are constructed in this case.\nFor Example:\n - `s3://my-object-store/file1`\n - `gs://my-bucket/file2`\n"
            " - `file:///path/to/my/file`"
        ),
    )
    path: str = Field(
        ...,
        description=(
            "Absolute path of the file inside the container.\nMay contain pattern matching wildcards to select multiple"
            " outputs at once, but mind\nimplications for `tesOutput.url` and `tesOutput.path_prefix`.\nOnly wildcards"
            " defined in IEEE Std 1003.1-2017 (POSIX), 12.3 are supported;"
    ...
    path_prefix: Optional[str] = Field(
        ...
        description=(
            ...
            " constructed by appending pruned paths to the directory specfied\nin `tesOutput.url`.\nRequired if"
            " `tesOutput.path` contains wildcards, ignored otherwise."
        ),
    )
    type: Optional[TesFileType] = "FILE"
...
class TesOutputFileLog(BaseModel):
    url: str = Field(..., description="URL of the file in storage, e.g. s3://bucket/file.txt")
    path: str = Field(
        ...,
...
class TesResources(BaseModel):
    cpu_cores: Optional[int] = Field(None, description="Requested number of CPUs", example=4)
    preemptible: Optional[bool] = Field(
        None,
        description=(
            "Define if the task is allowed to run on preemptible compute instances,\nfor example, AWS Spot. This option"
            " may have no effect when utilized\non some backends that don't have the concept of preemptible jobs."
        ),
        example=False,
    )
    ram_gb: Optional[float] = Field(None, description="Requested RAM required in gigabytes (GB)", example=8)
    disk_gb: Optional[float] = Field(None, description="Requested disk size in gigabytes (GB)", example=40)
    zones: Optional[List[str]] = Field(
        None,
        description=(
            "Request that the task be run in these compute zones. How this string\nis utilized will be dependent on the"
            " backend system. For example, a\nsystem based on a cluster queueing system may use this string to"
    ...
    backend_parameters: Optional[Dict[str, str]] = Field(
        None,
        description=(
            "Key/value pairs for backend configuration.\nServiceInfo shall return a list of keys that a backend"
            " supports.\nKeys are case insensitive.\nIt is expected that clients pass all runtime or hardware"
            ...
            " shall not store or return unsupported keys if included in a task.\nIf backend_parameters_strict equals"
    ...
    backend_parameters_strict: Optional[bool] = Field(
        False,
        description=(
            "If set to true, backends should fail the task if any backend_parameters\nkey/values are unsupported,"
            " otherwise, backends should attempt to run the task"
        ),
        example=False,
    )
...
class ServiceType(BaseModel):
    group: str = Field(
        ...,
        description=(
            "Namespace in reverse domain name format. Use `org.ga4gh` for implementations compliant with official GA4GH"
            " specifications. For services with custom APIs not standardized by GA4GH, or implementations diverging"
            " from official GA4GH specifications, use a different namespace (e.g. your organization's reverse domain"
            " name)."
        ),
        example="org.ga4gh",
    )
    artifact: str = Field(
        ...,
        description=(
            "Name of the API or GA4GH specification implemented. Official GA4GH types should be assigned as part of"
            " standards approval process. Custom artifacts are supported."
        ),
        example="beacon",
    )
    version: str = Field(
...
class Service(BaseModel):
    id: str = Field(
        ...,
        description=(
            "Unique ID of this service. Reverse domain name notation is recommended, though not required. The"
            " identifier should attempt to be globally unique so it can be used in downstream aggregator services e.g."
            " Service Registry."
        ),
    ...
    contactUrl: Optional[AnyUrl] = Field(
        None,
        description=(
            "URL of the contact for the provider of this service, e.g. a link to a contact form (RFC 3986 format), or"
            " an email (RFC 2368 format)."
        ),
    ...
    documentationUrl: Optional[AnyUrl] = Field(
        None,
        description=(
            "URL of the documentation of this service (RFC 3986 format). This should help someone learn how to use your"
            " service, including any specifics required to access data, e.g. authentication."
        ),
    ...
    environment: Optional[str] = Field(
        None,
        description=(
            "Environment the service is running in. Use this to distinguish between production, development and"
            " testing/staging deployments. Suggested values are prod, test, dev, staging. However this is advised and"
            " not enforced."
        ),
        example="test",
    )
    version: str = Field(
        ...,
        description=(
            "Version of the service being described. Semantic versioning is recommended, but other identifiers, such as"
            " dates or commit hashes, are also allowed. The version should be changed whenever the service is updated."
        ),
        example="1.0.0",
    )
...
class TesTaskLog(BaseModel):
    ...
    system_logs: Optional[List[str]] = Field(
        ...
        description=(
            ...
            " process.\nContent is implementation specific: format, size, etc.\n\nSystem logs may be collected here to"
            " provide convenient access.\n\nFor example, the system may include the name of the host\nwhere the task is"
            " executing, an error message that caused\na SYSTEM_ERROR state (e.g. disk is full), etc.\n\nSystem logs"
...
class TesTask(BaseModel):
    ...
    state: Optional[TesState] = "UNKNOWN"
    ...
    inputs: Optional[List[TesInput]] = Field(
        None,
        description=(
            "Input files that will be used by the task. Inputs will be downloaded\nand mounted into the executor"
            " container as defined by the task request\ndocument."
        ),
    ...
    logs: Optional[List[TesTaskLog]] = Field(
        ...
        description=(
            ...
            " and is retried, an entry will be appended to this list."
        ),
    )
...
class TesListTasksResponse(BaseModel):
    tasks: List[TesTask] = Field(
        ...,
        description=(
            "List of tasks. These tasks will be based on the original submitted\ntask document, but with other fields,"
            " such as the job state and\nlogging info, added/changed as the job progresses."
        ),
    )
    next_page_token: Optional[str] = Field(
        None,
        description=(
            "Token used to return the next page of results. This value can be used\nin the `page_token` field of the"
            " next ListTasks request."
        ),
    )


# Added manually from v1_0_specs.py as it seemed to be missed by datamodel-codegen
