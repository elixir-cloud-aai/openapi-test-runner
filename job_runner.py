...
class JobRunner():
    ...
    def generate_summary(self):
        print("\n\n\n")
        print_yellow("{:#^50}".format("   Compliance Testing Summary   "))
        print_yellow("#{:^48}#".format(""))
        print_yellow("#{:^48}#".format(f"Total Tests - {self.test_count}"))
        print_yellow("#{:^48}#".format(f'Passed - {self.result["passed"]}'))
        print_yellow("#{:^48}#".format(f'Failed - {self.result["failed"]}'))
        print_yellow("#{:^48}#".format(f'Skipped - {self.result["skipped"]}'))
        print_yellow("#{:^48}#".format(""))
        print_yellow("{:#^50}".format(""))
                    ...
                    try:
                        ...
                        for tag in self.tags:
                            if tag in yaml_data["tags"]:
                                tag_matched = True
                                test_runner = TestRunner(yaml_data["server"], yaml_data["version"][0], self.result)
                                job_count = 0
